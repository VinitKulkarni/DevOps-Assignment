  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const healthCheck = await axios.get(`http://backend-alb-1412117731.ap-south-1.elb.amazonaws.com/api/health`);
        
        if (healthCheck.data.status === 'healthy') {
          setStatus('Backend is connected!');
          // Then fetch the message
          const response = await axios.get(`http://backend-alb-1412117731.ap-south-1.elb.amazonaws.com/api/message`);
